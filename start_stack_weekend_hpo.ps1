<#
  start_stack_weekend_hpo.ps1

  Starts the complete forecasting stack for WEEKEND_HPO run:

  - Prometheus (separate PowerShell window)
  - FastAPI /metrics server (separate PowerShell window)
  - Forecast engine weekend HPO run (current window)

  Adjust the paths in the CONFIG section once, then you can just:
    - Right-click → "Run with PowerShell"
    - or run:  .\start_stack_weekend_hpo.ps1
#>
...
Write-Host "=== Starting Agentic Forecast Stack (WEEKEND_HPO) ===" -ForegroundColor Cyan
