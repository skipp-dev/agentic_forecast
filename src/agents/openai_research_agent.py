...
class OpenAIResearchAgent:
    ...
    def conduct_market_research(self, symbols: Optional[List[str]] = None, days_back: int = 7) -> ResearchInsights:
        ...
        try:
            ...
            research_insights = ResearchInsights(
                ...
                risk_assessment=insights.get('risk_assessment', 'Assessment unavailable'),
                trading_signals=insights.get('trading_signals', []),
                confidence_score=float(insights.get('confidence_score', 0.0)),
    ...
    def _analyze_news_sentiment(self, articles: List[NewsArticle]) -> List[NewsArticle]:
        ...
        for article in articles:
            try:
                ...
                article.sentiment_score = sentiment_data.get('sentiment_score', 0.0)
                article.sentiment_label = sentiment_data.get('sentiment_label', 'neutral')
                article.key_entities = sentiment_data.get('key_entities', [])
                article.impact_assessment = sentiment_data.get('impact_assessment', '')
    ...
    def _analyze_article_sentiment(self, article: NewsArticle) -> Dict[str, Any]:
        ...
        try:
            ...
            cleaned_response = extract_json_from_llm_output(response.strip())
